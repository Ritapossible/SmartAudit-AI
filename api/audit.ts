...
// Mirrors the Express route in artifacts/api-server/src/routes/audit.ts
...
function runSecurityValidator(code: string): ValidatorResult {
  ...
  if (
    ...
    findings.push({
      ...
      severity: "critical",
      explanation: "ETH is sent before updating the balance, allowing recursive withdraw calls to drain the contract.",
  ...
  if (/tx\.origin/.test(code)) {
    findings.push({
      ...
      severity: "high",
      explanation: "tx.origin can be manipulated in phishing attacks. Use msg.sender for authentication.",
  ...
  if (/delegatecall/.test(code) && /msg\.data|calldataload|_target|_addr/.test(code)) {
    findings.push({
      ...
      severity: "critical",
      explanation: "Delegatecall with a user-controlled address allows attackers to execute arbitrary code in your contract's context.",
  ...
  if (/selfdestruct|suicide\s*\(/.test(code) && !/onlyOwner|require\(msg\.sender/.test(code)) {
    findings.push({
      ...
      severity: "critical",
      explanation: "A selfdestruct call without access control allows anyone to permanently destroy the contract.",
  ...
  const score = Math.min(100, findings.reduce((s, f) =>
    s + (f.severity === "critical" ? 40 : f.severity === "high" ? 25 : f.severity === "medium" ? 15 : 5), 0));
...
function runGasValidator(code: string): ValidatorResult {
  ...
  if (storageReads > 5 && /for\s*\(/.test(code)) {
    findings.push({
      ...
      severity: "medium",
      explanation: "Reading from storage inside a loop is expensive (2,100 gas per cold read). Cache values in memory.",
  ...
  if (/uint8\s+\w+\s*=|uint16\s+\w+\s*=/.test(code)) {
    findings.push({
      ...
      severity: "low",
      explanation: "uint8/uint16 cost more gas than uint256 because the EVM must pad them to 32 bytes.",
  ...
  if (!/event\s+\w+/.test(code)) {
    findings.push({
      ...
      severity: "low",
      explanation: "State-changing functions emit no events, making off-chain indexing impossible and increasing long-term gas costs for observers.",
  ...
  if (/function\s+\w+[^{]*public[^{]*{[^}]*\bstorage\b/.test(code)) {
    findings.push({
      ...
      severity: "low",
      explanation: "Multiple writes to the same storage variable in one function are more expensive than batching.",
...
function runAccessControlValidator(code: string): ValidatorResult {
  ...
  const hasSetters = /function\s+set\w+|function\s+update\w+|function\s+change\w+/.test(code);

  if (hasSetters && !hasModifier) {
    findings.push({
      title: "Missing Access Control on Setters",
      severity: "high",
      explanation: "Public/external setter functions have no ownership or role check, allowing anyone to modify critical state.",
  ...
  if (!hasOwner) {
    findings.push({
      ...
      severity: "medium",
      explanation: "Contract has no ownership pattern, making it impossible to pause, upgrade, or control in an emergency.",
  ...
  if (/function\s+\w+\s*\([^)]*\)\s*public\s*payable/.test(code) && !hasModifier) {
    findings.push({
      ...
      severity: "high",
      explanation: "A payable function is publicly accessible without any access control, risking unintended ETH locking.",
      fix: "Restrict payable functions to authorized callers or add require(msg.sender == owner) checks.",
    });
  }

  if (/address\(0\)|address(0x0)/.test(code) && !/require.*!=\s*address\(0\)/.test(code)) {
    findings.push({
      title: "Missing Zero Address Validation",
      severity: "medium",
      explanation: "Addresses assigned without checking for the zero address (0x000...0) can permanently lock funds.",
  ...
  if (/ecrecover/.test(code) && !/nonce|nonces|used\[/.test(code)) {
    findings.push({
      ...
      severity: "high",
      explanation: "ecrecover usage without a nonce or replay protection allows the same signed message to be resubmitted multiple times.",
...
function runFormalValidator(code: string): ValidatorResult {
  ...
  if (!/pragma solidity 0\.[89]\./.test(code)) {
    findings.push({
      ...
      severity: "medium",
      explanation: "Using a pragma that allows versions older than 0.8 means the compiler won't guard against integer overflows/underflows.",
  ...
  if (/\+\+|\-\-|[^=!<>]=[^=]/.test(code) && !/SafeMath|unchecked/.test(code)) {
    findings.push({
      ...
      severity: "medium",
      explanation: "Arithmetic operations without explicit overflow checks or SafeMath could silently wrap in older Solidity versions.",
  ...
  if (/\^0\.[0-7]\./.test(code)) {
    findings.push({
      ...
      severity: "low",
      explanation: "Using a caret (^) pragma allows compilation with any future compatible release, which may introduce unexpected behavior.",
  ...
  if (/\/\d+/.test(code) && !/SafeMath|mulDiv/.test(code)) {
    findings.push({
      ...
      severity: "low",
      explanation: "Integer division in Solidity truncates results silently. For example, 3/2 equals 1, not 1.5.",
